...
class App:
    ...
    #def register_callbacks(self):
        @self.app.callback(
            ...
            State('ticker-input', 'value')) # Allows to share data between functions
...
def get_data(tickers):
        ...
        data_list =  [data.get_data_yahoo(tick , dt.datetime(1975,1,1)) for tick in tickers]
        return data_list
...
def plot_candle_adx(df):
        ''' Plot candlesticks of the last 12 years with the ADX , to assess the trend '''

        df['ADX'] = ta.ADX(df['High'] , df['Low'] , df['Close'] , timeperiod=14)
        df_ = df.copy()
        try:
            df_ = df_[df.index[-1] - dt.timedelta(252*12):]
        except:
            try:
                df_ = df_[:df.index[-1] - dt.timedelta(252*10):]
            except:
                try:
                    df_ = df_[:df.index[-1] - dt.timedelta(252*8):]
                except:
                    try:
                        df_ = df_[:df.index[-1] - dt.timedelta(252*5):]
                    except:
                        try:
                            df_ = df_[:df.index[-1] - dt.timedelta(252*2):]
                        except:
                            try:
                                df_ = df_[:df.index[-1] - dt.timedelta(252):]
                            except:
                                try:
                                    df_ = df_[:df.index[-1] - dt.timedelta(125):]
                                except:
                                    df_ = df_[:df.index[-1] - dt.timedelta(75):]
...
def prepare_for_decompose(df : pd.DataFrame):
...
def plot_seasonality(to_plot : pd.DataFrame , title : str , hover_data : str):
    ...
    fig.update_layout(
        title={
            'text': f"Breakdown of annual annual seasonality",
            'y':0.95,
            'x':0.5,
            'xanchor': 'center',
            'yanchor': 'top'})
...
def get_the_three_season(df : pd.DataFrame):
